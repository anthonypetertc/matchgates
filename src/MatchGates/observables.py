...
class Observable():
    def __init__(self, name: str, qubits: list[int], n_qubits: int):
        assert name in set(["Z", "ZZ", "XX", "XY", "YY", "YX"]), "Observable not implemented,\
        ...
        if name == "Z":
            ...
            assert len(qubits) == 2
            assert qubits[1] - qubits[0] == 1, "XX, YY, XY, YX observables must be nearest neighbour."
            assert (qubits[0] < n_qubits) and (qubits[1] < n_qubits),  f"Observable must apply to a qubit in a chain of length {n_qubits}"
        self.name = name
    ...
    def get_majoranas(self):
        ...
        if self.name == "ZZ":
            ...
            raise ValueError("Observable not implemented, \
                only allowed observables are Z, ZZ, XX, XY, YX, YY")
