...
def test_circuit():
    ...
    for i in range(n_gates):
        ...
        gate_list.append(AppliedMatchGate(MatchGate.randomMatchGate(), n_qubits = n_qubits, acts_on=(a, a+1)))

    circuit = Circuit(n_qubits=n_qubits, gate_list=gate_list) #make the circuit
    T = circuit.T

    state = ProductState([ops.q0, ops.q0, ops.q0, ops.q0]) #make initial product state |0000>
...
def test_add_noise():
    ...
    noisy_circuit = circuit.add_uniform_noise_to_circuit(sgn) # Add noise to the circuit.
...
def test_noisy_circuit():
    ...
    U1 = la.expm(1j * (
        theta * np.kron(X, X) + 
        phi * np.kron(Z, I) + phi * np.kron(I, Z)+
        gamma * np.kron(Y, Y) +
        kappa * np.kron(Y, X) +
        beta * np.kron(X, Y)
        ))
    ...
    n_qubits= 2
    gate_list = []
    gate_list.append(AppliedMatchGate(MatchGate.fromUnitary(U1), n_qubits = n_qubits, acts_on=(0, 1)))
    ...
    for noisy_gate in noisy_circuit.noisy_gate_list:
        ...
        kops = [AppliedMatchGate(op, amg.n_qubits, amg.acts_on).convert_to_unitary() for op in noise.ops]
        probs = noisy_gate.noise.probabilities
        gate = amg.convert_to_unitary()
        dm = gate @ dm @ gate.conj().transpose()
        dm = sum([(1 - sum(probs)) * dm] + [prob * kraus @ dm @ kraus.conj().transpose() for prob, kraus in zip(probs, kops)])
